...
namespace GooglePlayGames
{
    ...
    public class PlayGamesPlatform : ISocialPlatform
    {
        ...
        public static void InitializeInstance(PlayGamesClientConfiguration configuration)
        {
            if (sInstance != null)
            {
                Logger1.w("PlayGamesPlatform already initialized. Ignoring this call.");
                return;
            }
        ...
        public static PlayGamesPlatform Instance
        {
            get
            {
                if (sInstance == null)
                {
                    Logger1.d("Instance was not initialized, using default configuration.");
        ...
        public static bool DebugLogEnabled
        {
            get
            {
                return Logger1.DebugLogEnabled;
            }

            set
            {
                Logger1.DebugLogEnabled = value;
            }
        }
        ...
        {
            Logger1.d("Activating PlayGamesPlatform.");
            Social.Active = PlayGamesPlatform.Instance;
            Logger1.d("PlayGamesPlatform activated: " + Social.Active);
        ...
        public void Authenticate(Action<bool> callback, bool silent)
        {
            ...
            if (mClient == null)
            {
                Logger1.d("Creating platform-specific Play Games client.");
        ...
        {
            Logger1.w("PlayGamesPlatform.LoadUsers is not implemented.");
            if (callback != null)
            {
        ...
        public string GetUserId()
        {
            if (!IsAuthenticated())
            {
                Logger1.e("GetUserId() can only be called after authentication.");
                return "0";
            }
        ...
        public Achievement GetAchievement(string achievementId) {
            if (!IsAuthenticated()) {
                Logger1.e("GetAchievement can only be called after authentication.");
                return null;
            }
        ...
        public string GetUserDisplayName()
        {
            if (!IsAuthenticated())
            {
                Logger1.e("GetUserDisplayName can only be called after authentication.");
                return string.Empty;
            }
        ...
        public string GetUserImageUrl()
        {
            if (!IsAuthenticated())
            {
                Logger1.e("GetUserImageUrl can only be called after authentication.");
                return null;
            }
        ...
        public void ReportProgress(string achievementID, double progress, Action<bool> callback)
        {
            if (!IsAuthenticated())
            {
                Logger1.e("ReportProgress can only be called after authentication.");
                if (callback != null)
                {
            ...
            Logger1.d("ReportProgress, " + achievementID + ", " + progress);
            achievementID = MapId(achievementID);

            // if progress is 0.0, we just want to reveal it
            if (progress < 0.000001)
            {
                Logger1.d("Progress 0.00 interpreted as request to reveal.");
            ...
            if (ach == null)
            {
                Logger1.w("Unable to locate achievement " + achievementID);
                Logger1.w("As a quick fix, assuming it's standard.");
                isIncremental = false;
            }
            else
            {
                ...
                Logger1.d("Achievement is " + (isIncremental ? "INCREMENTAL" : "STANDARD"));
                if (isIncremental)
                {
                    Logger1.d("Current steps: " + curSteps + "/" + totalSteps);
                }
            }
            ...
            if (isIncremental)
            {
                ...
                if (progress >= 0.0 && progress <= 1.0)
                {
                    ...
                    Logger1.w("Progress " + progress + " is less than or equal to 1. You might be trying to use values in the range of [0,1], while values are expected to be within the range [0,100]. If you are using the latter, you can safely ignore this message.");
                }

                int targetSteps = (int)((progress / 100) * totalSteps);
                int numSteps = targetSteps - curSteps;
                Logger1.d("Target steps: " + targetSteps + ", cur steps:" + curSteps);
                Logger1.d("Steps to increment: " + numSteps);
                if (numSteps > 0)
                {
            ...
            {
                // unlock it!
                Logger1.d("Progress " + progress + " interpreted as UNLOCK.");
                mClient.UnlockAchievement(achievementID, callback);
            }
            else
            {
                // not enough to unlock
                Logger1.d("Progress " + progress + " not enough to unlock non-incremental achievement.");
            }
        }
        ...
        public void IncrementAchievement(string achievementID, int steps, Action<bool> callback)
        {
            if (!IsAuthenticated())
            {
                Logger1.e("IncrementAchievement can only be called after authentication.");
                if (callback != null)
                {
                    callback.Invoke(false);
                }
            ...
            // map ID, if it's in the dictionary
            Logger1.d("IncrementAchievement: " + achievementID + ", steps " + steps);
            achievementID = MapId(achievementID);
        ...
        {
            Logger1.w("PlayGamesPlatform.LoadAchievementDescriptions is not implemented.");
            if (callback != null)
            {
        ...
        {
            Logger1.w("PlayGamesPlatform.LoadAchievements is not implemented.");
            if (callback != null)
            {
        ...
        public void ReportScore(long score, string board, Action<bool> callback)
        {
            if (!IsAuthenticated())
            {
                Logger1.e("ReportScore can only be called after authentication.");
                if (callback != null)
                {
                    callback.Invoke(false);
                }
            ...
            Logger1.d("ReportScore: score=" + score + ", board=" + board);
        ...
        {
            Logger1.w("PlayGamesPlatform.LoadScores not implemented.");
            if (callback != null)
            {
        ...
        {
            Logger1.w("PlayGamesPlatform.CreateLeaderboard not implemented. Returning null.");
            return null;
        }
        ...
        public void ShowAchievementsUI()
        {
            if (!IsAuthenticated())
            {
                Logger1.e("ShowAchievementsUI can only be called after authentication.");
                return;
            }

            Logger1.d("ShowAchievementsUI");
        ...
        public void ShowLeaderboardUI()
        {
            if (!IsAuthenticated())
            {
                Logger1.e("ShowLeaderboardUI can only be called after authentication.");
                return;
            }

            Logger1.d("ShowLeaderboardUI");
        ...
        public void ShowLeaderboardUI(string lbId)
        {
            ...
            Logger1.d("ShowLeaderboardUI, lbId=" + lbId);
            if (lbId != null)
            {
                lbId = MapId(lbId);
            }
        ...
        {
            Logger1.d("SetDefaultLeaderboardForUI: " + lbid);
            if (lbid != null)
            {
                lbid = MapId(lbid);
            }
        ...
        {
            Logger1.w("PlayGamesPlatform.LoadFriends not implemented.");
            if (callback != null)
            {
                callback.Invoke(false);
            }
        ...
        public void LoadState(int slot, OnStateLoadedListener listener)
        {
            if (!IsAuthenticated())
            {
                Logger1.e("LoadState can only be called after authentication.");
                if (listener != null)
                {
        ...
        public void UpdateState(int slot, byte[] data, OnStateLoadedListener listener)
        {
            if (!IsAuthenticated())
            {
                Logger1.e("UpdateState can only be called after authentication.");
                if (listener != null)
                {
        ...
        private string MapId(string id)
        {
            ...
            if (mIdMap.ContainsKey(id))
            {
                ...
                Logger1.d("Mapping alias " + id + " to ID " + result);
                return result;
            }
