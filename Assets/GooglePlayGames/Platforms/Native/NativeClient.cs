    ...
    public class NativeClient : IPlayGamesClient
    {
        ...
        private static void InvokeCallbackOnGameThread(Action<bool> callback, bool success)
        {
            ...
            PlayGamesHelperObject.RunOnGameThread(() =>
                {
                    Logger1.d("Invoking user callback on game thread");
                    callback(success);
                });
        }
                                       ...
                                       MultiplayerInvitation invitation)
        {
            ...
            {
                Logger1.d("Received " + eventType + " for invitation "
            ...
            {
                Logger1.d("Ignoring REMOVED for invitation " + invitationId);
                return;
            }
                                          ...
                                          AchievementManager.FetchAllResponse response)
        {
            ...
            {
                Logger1.d("Received achievement callback after signout occurred, ignoring");
                return;
            }

            Logger1.d("Populating Achievements");
            lock (AuthStateLock)
            {
                if (response.Status() != Status.ResponseStatus.VALID &&
                    response.Status() != Status.ResponseStatus.VALID_BUT_STALE)
                {
                    Logger1.e("Error retrieving achievements - check the log for more information. " +
                        "Failing signin.");
            ...
            Logger1.d("Maybe finish for Achievements");
        ...
        void MaybeFinishAuthentication()
        {
            ...
            lock (AuthStateLock)
            {
                ...
                {
                    Logger1.d("Auth not finished. User=" + mUser + " achievements=" + mAchievements);
                    return;
                }

                Logger1.d("Auth finished. Proceeding.");
            ...
            {
                Logger1.d("Invoking Callbacks: " + localCallbacks);
        ...
        {
            Logger1.d("Populating User");

            if (authGeneration != mAuthGeneration)
            {
                Logger1.d("Received user callback after signout occurred, ignoring");
                return;
            }
            ...
            lock (AuthStateLock)
            {
                if (response.Status() != Status.ResponseStatus.VALID &&
                    response.Status() != Status.ResponseStatus.VALID_BUT_STALE)
                {
                    Logger1.e("Error retrieving user, signing out");
            ...
            Logger1.d("Found User: " + mUser);
            Logger1.d("Maybe finish for User");
            MaybeFinishAuthentication();
        }

        private void HandleAuthTransition(Types.AuthOperation operation, Status.AuthStatus status)
        {
            Logger1.d("Starting Auth Transition. Op: " + operation + " status: " + status);
            lock (AuthStateLock)
            {
                switch (operation)
                {
                    ...
                    case Types.AuthOperation.SIGN_OUT:
                        ...
                        break;
                    default:
                        Logger1.e("Unknown AuthOperation " + operation);
                        break;
                }
            }
        }
                                       ...
                                       Predicate<Achievement> alreadyDone, Action<Achievement> updateAchievment)
        {
            ...
            if (achievement == null)
            {
                Logger1.d("Could not " + updateType + ", no achievement with ID " + achId);
                callback(false);
                return;
            }
            ...
            {
                Logger1.d("Did not need to perform " + updateType + ": " + "on achievement " + achId);
                callback(true);
                return;
            }

            Logger1.d("Performing " + updateType + " on " + achId);
        ...
        public void IncrementAchievement(string achId, int steps, Action<bool> callback)
        {
            ...
            if (achievement == null)
            {
                Logger1.e("Could not increment, no achievement with ID " + achId);
                callback(false);
                return;
            }
            ...
            {
                Logger1.e("Could not increment, achievement with ID " + achId + " was not incremental");
                callback(false);
                return;
            }
            ...
            if (steps < 0)
            {
                Logger1.e("Attempted to increment by negative steps");
                callback(false);
                return;
            }
        ...
        public void LoadState(int slot, OnStateLoadedListener listener)
        {
            ...
            lock (GameServicesLock)
            {
                if (mAuthState != AuthState.Authenticated)
                {
                    Logger1.e("You can only call LoadState after the user has successfully logged in");
        ...
        public void UpdateState(int slot, byte[] data, OnStateLoadedListener listener)
        {
            ...
            lock (GameServicesLock)
            {
                if (mAuthState != AuthState.Authenticated)
                {
                    Logger1.e("You can only call UpdateState after the user has successfully logged in");
