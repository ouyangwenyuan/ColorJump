    ...
    public class NativeRealtimeMultiplayerClient : IRealTimeMultiplayerClient
    {
                                ...
                                RealTimeMultiplayerListener listener)
        {
            lock (mSessionLock)
            {
                ...
                {
                    Logger1.e("Received attempt to create a new room without cleaning up the old one.");
            ...
            lock (mSessionLock)
            {
                ...
                mRealtimeManager.ShowPlayerSelectUI(minOpponents, maxOppponents, true,
                    response =>
                    {
                        ...
                        {
                            Logger1.d("User did not complete invitation screen.");
                            newRoom.LeaveRoom();
                            return;
                        }
        ...
        public void AcceptFromInbox(RealTimeMultiplayerListener listener)
        {
            lock (mSessionLock)
            {
                ...
                if (mCurrentSession.IsActive())
                {
                    Logger1.e("Received attempt to accept invitation without cleaning up " +
                        "active session.");
                    newRoom.LeaveRoom();
                    return;
                }
                ...
                mRealtimeManager.ShowRoomInboxUI(
                    response =>
                    {
                        ...
                        using (var helper = HelperForSession(newRoom))
                        {
                            Logger1.d("About to accept invitation " + invitation.Id());
                            newRoom.StartRoomCreation(mNativeClient.GetUserId(),
                ...
                mRealtimeManager.FetchInvitations(response =>
                    {
                        ...
                        {
                            Logger1.e("Couldn't load invitations.");
                            newRoom.LeaveRoom();
                            return;
                        }
                        ...
                        Logger1.e("Room creation failed since we could not find invitation with ID "
                            + invitationId);
                        newRoom.LeaveRoom();
                    });
            }
         ...
         */
            ...
            internal void EnterState(State handler)
            {
                lock (mLifecycleLock)
                {
                    ...
                    Logger1.d("Entering state: " + handler.GetType().Name);
            ...
            internal void StartRoomCreation(string currentPlayerId, Action createRoom)
            {
                lock (mLifecycleLock)
                {
                    ...
                    {
                        Logger1.e("Room creation started more than once, this shouldn't happen!");
                        return;
                    }

                    if (!mState.IsActive())
                    {
                        Logger1.w("Received an attempt to create a room after the session was already " +
                            "torn down!");
                        return;
                    }
        ...
        internal abstract class State
        {
            ...
            {
                Logger1.d(this.GetType().Name + ".HandleRoomResponse: Defaulting to no-op.");
            }

            internal virtual bool IsActive()
            {
                Logger1.d(this.GetType().Name + ".IsNonPreemptable: Is preemptable by default.");
                return true;
            }

            internal virtual void LeaveRoom()
            {
                Logger1.d(this.GetType().Name + ".LeaveRoom: Defaulting to no-op.");
            }

            internal virtual void OnStateEntered()
            {
                Logger1.d(this.GetType().Name + ".OnStateEntered: Defaulting to no-op.");
            }

            internal virtual void OnRoomStatusChanged(NativeRealTimeRoom room)
            {
                Logger1.d(this.GetType().Name + ".OnRoomStatusChanged: Defaulting to no-op.");
            }

            internal virtual void OnConnectedSetChanged(NativeRealTimeRoom room)
            {
                Logger1.d(this.GetType().Name + ".OnConnectedSetChanged: Defaulting to no-op.");
            }

            internal virtual void OnParticipantStatusChanged(NativeRealTimeRoom room,
                                                         MultiplayerParticipant participant)
            {
                Logger1.d(this.GetType().Name + ".OnParticipantStatusChanged: Defaulting to no-op.");
            }

            internal virtual void OnDataReceived(NativeRealTimeRoom room, MultiplayerParticipant sender,
                                             byte[] data, bool isReliable)
            {
                Logger1.d(this.GetType().Name + ".OnDataReceived: Defaulting to no-op.");
            }

            internal virtual void SendToSpecificRecipient(
                string recipientId, byte[] data, int offset, int length, bool isReliable)
            {
                Logger1.d(this.GetType().Name + ".SendToSpecificRecipient: Defaulting to no-op.");
            }

            internal virtual void SendToAll(byte[] data, int offset, int length, bool isReliable)
            {
                Logger1.d(this.GetType().Name + ".SendToApp: Defaulting to no-op.");
            }

            internal virtual List<Participant> GetConnectedParticipants()
            {
                Logger1.d(this.GetType().Name + ".GetConnectedParticipants: Returning empty connected" +
                    " participants");
            ...
            internal virtual Participant GetSelf()
            {
                Logger1.d(this.GetType().Name + ".GetSelf: Returning null self.");
                return null;
            }

            internal virtual Participant GetParticipant(string participantId)
            {
                Logger1.d(this.GetType().Name + ".GetSelf: Returning null participant.");
                return null;
            }

            internal virtual bool IsRoomConnected()
            {
                Logger1.d(this.GetType().Name + ".IsRoomConnected: Returning room not connected.");
                return false;
            }
        }
        ...
        private abstract class MessagingEnabledState : State
        {
                ...
                string recipientId, byte[] data, int offset, int length, bool isReliable)
            {
                ...
                {
                    Logger1.e("Attempted to send message to unknown participant " + recipientId);
                    return;
                }
        ...
        class BeforeRoomCreateStartedState : State
        {
            ...
            {
                Logger1.d("Session was torn down before room was created.");
        ...
        class RoomCreationPendingState : State
        {
            ...
            internal override void LeaveRoom()
            {
                Logger1.d("Received request to leave room during room creation, aborting creation.");
        ...
        class ConnectingState : MessagingEnabledState
        {
            ...
            internal override void HandleConnectedSetChanged(NativeRealTimeRoom room)
            {
                ...
                {
                    Logger1.w("Received connected set callback with unchanged connected set!");
                    return;
                }
                ...
                {
                    Logger1.e("Participants disconnected during room setup, failing. " +
                ...
                Logger1.d("New participants connected: " +
                ...
                {
                    Logger1.d("Fully connected! Transitioning to active state.");
                                                              ...
                                                              MultiplayerParticipant participant)
            {
                ...
                Logger1.e(string.Format("Participant {0} changed to status {1}, room will never be" +
        ...
        class ActiveState : MessagingEnabledState
        {
            ...
            internal override void OnStateEntered()
            {
                if (GetSelf() == null)
                {
                    Logger1.e("Room reached active state with unknown participant for the player");
                    LeaveRoom();
                }
            }
            ...
            internal override Participant GetParticipant(string participantId)
            {
                ...
                {
                    Logger1.e("Attempted to retrieve unknown participant " + participantId);
                    return null;
                }
            ...
            internal override void HandleConnectedSetChanged(NativeRealTimeRoom room)
            {
                ...
                Logger1.d("Updated participant statuses: " +
                ...
                {
                    Logger1.w("Player was disconnected from the multiplayer session.");
                }
