...
internal class AndroidAppStateClient : AppStateClient {
    ...
    public void UpdateState(int slot, byte[] data, OnStateLoadedListener listener) {
        Logger1.d("UpdateState, slot=" + slot);
        using (var apiClient = GetApiClient(mServices)) {
    ...
    private class OnStateResultProxy : AndroidJavaProxy {
        ...
        private void OnStateConflict(int stateKey, string resolvedVersion,
            ...
            Logger1.d("OnStateResultProxy.onStateConflict called, stateKey=" + stateKey +
            ...
            if (mListener != null) {
                Logger1.d("OnStateResultProxy.onStateConflict invoking conflict callback.");
                ...
                Logger1.w("No conflict callback specified! Cannot resolve cloud save conflict.");
            }
        }

        private void ResolveState(int slot, string resolvedVersion, byte[] resolvedData,
            OnStateLoadedListener listener) {
            Logger1.d(string.Format("AndroidClient.ResolveState, slot={0}, ver={1}, " +
        ...
        private void OnStateLoaded(int statusCode, int stateKey, byte[] localData) {
            Logger1.d("OnStateResultProxy.onStateLoaded called, status " + statusCode +
                ", stateKey=" + stateKey);
            debugLogData("localData", localData);
            ...
            switch (statusCode) {
                case STATUS_OK:
                    Logger1.d("Status is OK, so success.");
                    success = true;
                    break;
                case STATUS_NO_DATA:
                    Logger1.d("Status is NO DATA (no network?), so it's a failure.");
                    success = false;
                    localData = null;
                    break;
                case STATUS_STALE_DATA:
                    Logger1.d("Status is STALE DATA, so considering as success.");
                    success = true;
                    break;
                case STATUS_KEY_NOT_FOUND:
                    Logger1.d("Status is KEY NOT FOUND, which is a success, but with no data.");
                    success = true;
                    localData = null;
                    break;
                default:
                    Logger1.e("Cloud load failed with status code " + statusCode);
                    success = false;
                    localData = null;
                    break;
            }
            ...
            if (mListener != null) {
                Logger1.d("OnStateResultProxy.onStateLoaded invoking load callback.");
                PlayGamesHelperObject.RunOnGameThread(() => {
                    mListener.OnStateLoaded(success, stateKey, localData);
                });
            } else {
                Logger1.w("No load callback specified!");
            }
        }

        private void debugLogData(string tag, byte[] data) {
            Logger1.d("   " + tag + ": " + Logger1.describe(data));
        }

        public void onResult(AndroidJavaObject result) {
            Logger1.d("OnStateResultProxy.onResult, result=" + result);

            int statusCode = GetStatusCode(result);
            Logger1.d("OnStateResultProxy: status code is " + statusCode);

            if (result == null) {
                Logger1.e("OnStateResultProxy: result is null.");
                return;
            }

            Logger1.d("OnstateResultProxy: retrieving result objects...");
            ...
            Logger1.d("Got result objects.");
            Logger1.d("loadedResult = " + loadedResult);
            Logger1.d("conflictResult = " + conflictResult);

            if (conflictResult != null) {
                Logger1.d("OnStateResultProxy: processing conflict.");
                ...
                Logger1.d("OnStateResultProxy: conflict args parsed, calling.");
                OnStateConflict(stateKey, ver, localData, serverData);
            } else if (loadedResult != null) {
                Logger1.d("OnStateResultProxy: processing normal load.");
                int stateKey = loadedResult.Call<int>("getStateKey");
                byte[] localData = ToByteArray(loadedResult.NullSafeCall("getLocalData"));
                Logger1.d("OnStateResultProxy: loaded args parsed, calling.");
                OnStateLoaded(statusCode, stateKey, localData);
            } else {
                Logger1.e("OnStateResultProxy: both loadedResult and conflictResult are null!");
            }
        }
    }
